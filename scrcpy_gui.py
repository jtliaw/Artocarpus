...
class PhoneFileBrowser(tk.Toplevel):
    ...
    def _insert_items(self, folders, files):
        for item in folders: self.tree.insert("", "end", values=(f"📁 {item}",))
        for item in files: self.tree.insert("", "end", values=(f"📄 {item}",))
    ...
    def _on_item_double_click(self, event):
        ...
        if item_text_with_icon.startswith("📁"):
            item_text = item_text_with_icon[2:]
    ...
    def _on_ok(self):
        ...
        if item_id:
            item_text_with_icon = self.tree.item(item_id, "values")[0]
            item_text = item_text_with_icon[2:]
            selected_path += item_text
        self.callback(selected_path.rstrip('/'))
        self.destroy()
...
# --- 语言字典 ---
LANGUAGES = {
    "app_title": {"zh": "Artocarpus (Scrcpy 图形界面) v5.0", "en": "ARtocarpus (Scrcpy GUI) v5.0"},
